...
export function createTools({ config, base, author, home, request }) {
  ...
  return [
    ...
    },
    {
      ...
      description: 'Run a card (model) on a quest now. This starts a real worker and may spend money; the owner normally does this by dragging on the board. Refusals come back with every reason. As the coordinator you may only dispatch the machine-check fast track: a card the owner marked coordinatorAssignable (free cards only) on a quest whose origin is machine-check or post-delivery-check, touching at most 3 files — anything else is refused, and the owner dispatches it himself. Always pass a requestKey you made up for this attempt: if the call is retried with the same key, the board answers with the existing attempt instead of starting a second worker. Pass ifRevision (from get_quest) so a quest that changed since you read it is refused as stale.',
