...
describe('variant capability eligibility and dispatch', () => {
  it('uses the same refusal before assign and at assign, carries unknown warnings in eligibility and dispatched detail', async () => {
    ...
    try {
      ...
      assert.equal(preDrop.reasons.find((reason) => reason.code === 'variant_unsupported').message, '这张卡的模型不支持变体「high」：到名册里清空这张卡的变体，或换一张支持它的卡');
      ...
      assert.deepEqual(verdict.warnings, [{ code: 'variant_unconfirmed', message: '还没确认这张卡支持变体「high」，照常派遣' }]);
      ...
      assert.match(dispatched.detail, /还没确认这张卡支持变体「high」，照常派遣/);
    } finally {
      await fx.close();
    }
  });
