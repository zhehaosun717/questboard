...
- Events file: one JSON line per change, `{seq, at, event, package, lane, model, variant, name, attemptId,
  ...
  cancel_acknowledged, cancel_result, review_override, metadata_update, check_failed, integrate, status_<status> (any other quest status, e.g.
...
7. From using it: a taken port is not a dead end — the desktop app names what holds it (another project's
   board, or a program's image and PID) and offers to move the project to a free port through
   `questboard port <n>` (timestamped backup kept), open the board that already runs, or end the holding
   program; a ruling closes the question threads it answers; a card may carry its own non-secret `env`
