...
| `policy`（对象） | `policy` 必须是一个对象：整个省略就用默认值，但显式写 `"policy": null`（或字符串、数组）会被拒绝，不再静默回退到默认值 |
...
`cancel_requested`、`cancel_acknowledged`、`cancel_result`、`review_override`、`metadata_update`、`check_failed`、`integrate`、`status_<状态>`（其它任何
委托状态）。
